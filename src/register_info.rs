...
pub const REGISTER_INFOS: [RegInfo; 125] = [
    gpr64!(rax, 0),
    gpr64!(rdx, 1),
    gpr64!(rcx, 2),
    gpr64!(rbx, 3),
...
impl RegInfo {
    pub fn val_as_i64(&self, val: &ValUnion) -> i64 {
        if self.format == RegisterFormat::Uint {
            let val = unsafe {
                match self.size {
                    1 => val.u8 as usize,
                    2 => val.u16 as usize,
                    4 => val.u32 as usize,
                    8 => val.u64 as usize,
                    _ => panic!("unknown uint size in val_as_i64")
                }
            };
            return val as i64;
        } else if self.format == RegisterFormat::Double {
            panic!("val_as_i64 does not support doubles");
        } else if self.format == RegisterFormat::LongDouble {
            panic!("val_as_i64 does not support long doubles");
        } else {
            panic!("unsupported rtype in val_as_i64");
        };
    }
}

fn register_find_by(f: impl Fn(&&RegInfo) -> bool) -> Result<&'static RegInfo> {
    let res = REGISTER_INFOS.iter().find(f);
    if res.is_none() {
        return error("could not find register");
    }
    Ok(res.unwrap())
}
